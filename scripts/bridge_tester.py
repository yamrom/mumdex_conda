...
def handler(signum, frame):
    print("pipe closed in bridge_tester.py", file=sys.stderr)
    sys.exit(1)
...
if len(sys.argv) == 2:
    ...
    print("usage: bridge_tester.py whole_genome", file=sys.stderr)
    exit(1)
...
files = list()
for sample in range(0, pop.n_samples()):
...
while True:
    ...
    if more_lines == False:
        break
    print(info[0]["chr"], info[0]["pos"], info[0]["out"], end=' ')
    for item in sorted(
        list(all_invariants.items()),
